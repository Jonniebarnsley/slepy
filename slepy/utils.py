...
def scale_factor(data: Union[DataArray, Dataset], sgn: int) -> DataArray:
    ...
    # Author's note: fine for Antarctica, since if the origin is not the bottom left corner
    # then it's probably on the pole. However, this may cause problems for Greenland. If you
    # are using this for Greenland, then I recommend avoiding the scale factor correction by
    # leaving pole=None (as is the default) in SLECalculator.
    xs = x - x.mean()
    ys = y - y.mean()
